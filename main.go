...
func main() {
	...
	prerecording := flag.Int64("prerecording", 5, "if motion based, pre recording is used to record a number of seconds before the event.")
	postrecording := flag.Int64("postrecording", 10, "if motion based, post recording is used to record a number of seconds after the event.")
	maxlengthrecording := flag.Int64("maxlengthrecording", 30, "the total (max) length/duration of a single recording.")
	flag.Parse()
	...
	if *action == "create" {
		...
		for i := 0; i < *numberOfAgents; i++ {
			...
			containerDetails.PreRecording = *prerecording
			containerDetails.PostRecording = *postrecording
			containerDetails.MaxLengthRecording = *maxlengthrecording
			containerDetails.Region = *region
			containerDetails.Region = *region
